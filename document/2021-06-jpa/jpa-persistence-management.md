...
예를들어 엔티티A와 B가 매핑되어 있을 때 각각 1000개씩 저장되어 있다면 Join을 사용할경우 최대 1,000,000 건의 조회가 이루어지겠지만 지연 로딩을 사용하여 1000건만 조회할 수 있습니다.   
...
3. JPQL 실행 시 flush 자동 호출
  ...
  만약 JPQL이 실행되기 전에 영속화된 엔티티가 있는경우, 해당 엔티티는 flush가 되지 않았으므로  
  영속성 컨텍스트에만 존재하고 데이터베이스에는 존재하지 않습니다.  
